  ...
  <Dialog :open="isOpen" @update:open="handleClose">
    <DialogContent class="sm:max-w-[600px]">
      ...
      <div class="grid gap-4 py-4">
        ...
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="unlock_conditions" class="text-right">Условия разблокировки (JSON)</Label>
          <Textarea id="unlock_conditions" v-model="unlockConditionsString" class="col-span-3" :class="{ 'border-red-500': !isUnlockConditionsValid }" />
        </div>
        <div class="grid grid-cols-4 items-center gap-4">
...
const unlockConditionsString = ref('{}');
const isUnlockConditionsValid = ref(true);
...
watch(() => props.isOpen, (newVal) => {
  if (newVal) {
    if (isEditing.value) {
      ...
      unlockConditionsString.value = props.rank.unlock_conditions ? JSON.stringify(props.rank.unlock_conditions, null, 2) : '{}';
    } else {
      form.value = {
        title: '',
        description: '',
      ...
      unlockConditionsString.value = '{}';
    }
    imageFile.value = null;
    isUnlockConditionsValid.value = true;
  }
});

watch(unlockConditionsString, (newVal) => {
  try {
    JSON.parse(newVal);
    isUnlockConditionsValid.value = true;
  } catch (e) {
    isUnlockConditionsValid.value = false;
  }
});
...
const handleSubmit = () => {
  if (!isUnlockConditionsValid.value) {
    errorMessage('Условия разблокировки содержат невалидный JSON.');
    return;
  }

  const rankData = { ...form.value };
  rankData.unlock_conditions = unlockConditionsString.value;
  if (imageFile.value) {
